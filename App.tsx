...
const App: React.FC = () => {
  ...
  // ------------------------------------------------------
  const handleExportSessions = useCallback(async (sessionsToExport: ParsedSession[]) => {
      ...
      try {
          ...
          const a = document.createElement('a');
          a.href = url;
          a.download = `Export_Parcial_${new Date().toISOString().slice(0,10)}.zip`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
                ...
                try {
                    ...
                    if (projectFile) {
                        ...
                        if (manifestFile) {
                            ...
                            const taggedSessions = zipSessions.map(s => ({ ...s, zipSourceId: zipId }));
                            newLoadedSessions.push(...taggedSessions);
                        }
                    }
                } catch (e) {
  ...
  const renderHome = () => (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-8 animate-fade-in relative z-10">
        <div className="text-center max-w-2xl mx-auto mb-4">
            ...
            Legal<span className="text-blue-600 dark:text-blue-500">Transcript</span> AI
            </h1>
        ...
        {sessions.length > 0 && (
            <button 
                ...
                className="group relative px-8 py-4 bg-slate-900 dark:bg-slate-800 text-white rounded-2xl shadow-xl hover:shadow-2xl hover:bg-slate-800 dark:hover:bg-slate-700 transition-all transform hover:-translate-y-1 w-full max-w-md flex items-center justify-between border border-slate-700 dark:border-slate-600"
            >
                <div className="text-left">
                    <div className="text-xs text-slate-400 dark:text-blue-400 uppercase font-bold tracking-wider mb-1">Trabalho em Curso</div>
                ...
                <div className="w-12 h-12 bg-slate-800 dark:bg-slate-700 rounded-full flex items-center justify-center group-hover:bg-blue-600 transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-4xl px-4">
            {/* New Process Card */}
            <div className="group bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 hover:shadow-2xl dark:hover:border-blue-500/50 cursor-pointer transition-all transform hover:-translate-y-1 flex flex-col items-center text-center relative">
                ...
                <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Novo Processo</h3>
                <p className="text-slate-500 dark:text-slate-400">
                    Carregar PDFs "brutos". <br/>
                    <span className="text-xs text-blue-600 dark:text-blue-300 font-bold bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded mt-2 inline-block">Suporta Múltiplos Ficheiros</span>
                </p>
            </div>

            {/* Load Project Card */}
            <div className="relative group bg-white dark:bg-slate-900 p-8 rounded-2xl shadow-xl border border-slate-100 dark:border-slate-800 hover:shadow-2xl dark:hover:border-purple-500/50 transition-all transform hover:-translate-y-1 flex flex-col items-center text-center overflow-hidden">
                ...
                <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Explorar / Carregar</h3>
                <p className="text-slate-500 dark:text-slate-400 mb-6">Carregue Projetos ZIP, pastas ou PDFs.</p>
                
                <div className="grid grid-cols-2 gap-4 w-full z-20">
                    <label className="flex flex-col items-center justify-center px-4 py-2 bg-purple-50 hover:bg-purple-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-purple-700 dark:text-slate-200 border border-purple-200 dark:border-slate-700 rounded-lg cursor-pointer transition-colors">
                         <span className="text-sm font-bold"><i className="fa-solid fa-file-zipper mr-2 dark:text-purple-400"></i>Projeto ZIP</span>
                         <input type="file" accept=".zip,.pdf" multiple onChange={handleExplorerLoad} className="hidden"/>
                    </label>
                    <label 
                        className="flex flex-col items-center justify-center px-4 py-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer transition-colors"
                        onClick={() => folderInputRef.current?.click()}
                    >
                         <span className="text-sm font-bold"><i className="fa-regular fa-folder-open mr-2 dark:text-yellow-400"></i>Pasta</span>
         ...
         <button onClick={() => setShowHelp(true)} className="mt-4 text-sm text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:hover:text-blue-400 font-medium flex items-center gap-2">
  ...
  const renderProcessor = () => (
     <div className="w-full max-w-3xl mx-auto relative z-10">
        <div className="flex items-center justify-between mb-8">
            <button onClick={() => setMode('HOME')} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white flex items-center gap-2 font-medium transition-colors">
                <i className="fa-solid fa-arrow-left"></i> Voltar
            </button>
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Processamento de PDF</h2>
        ...
        <div className="p-8 sm:p-12 text-center bg-slate-50/50 dark:bg-transparent">
          {!downloadUrl ? (
            <div className="space-y-6">
               {status.step !== 'IDLE' && status.step !== 'ERROR' ? (
                   <div className="text-slate-500 dark:text-slate-400">A processar...</div>
               ) : (
                <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-xl p-10 hover:bg-blue-50 dark:hover:bg-slate-800 hover:border-blue-400 dark:hover:border-blue-500/50 transition-all group cursor-pointer relative">
                    ...
                    <div className="w-16 h-16 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-500 rounded-full flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
                    <i className="fa-solid fa-file-pdf text-2xl"></i>
                    </div>
                    <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">Carregar PDF(s) Original(ais)</h3>
                </div>
               )}
            </div>
          ) : (
            <div className="py-8 animate-fade-in">
              ...
              <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Processamento Concluído!</h2>
              <div className="flex flex-col sm:flex-row gap-4 justify-center mt-6">
                <a href={downloadUrl} download={`${stats?.fileName}.zip`} className="inline-flex items-center justify-center gap-3 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold shadow-lg shadow-blue-200 dark:shadow-blue-900/50 transition-all">
                    <i className="fa-solid fa-file-zipper"></i> Descarregar ZIP
                </a>
                <button onClick={() => setMode('EXPLORER')} className="inline-flex items-center justify-center gap-3 px-6 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-xl font-bold shadow-sm transition-all">
                    <i className="fa-solid fa-magnifying-glass"></i> Explorar Resultados
                </button>
              </div>
              <button onClick={() => { setDownloadUrl(null); setStatus({ step: 'IDLE', message: '', progress: 0 }); }} className="block mx-auto mt-6 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 text-sm font-medium">Processar outros ficheiros</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col py-8 px-4 sm:px-6 lg:px-8 font-sans transition-colors duration-300">
      
      {/* THEME TOGGLE BUTTON */}
      <div className="fixed top-4 right-4 z-50">
        <button 
          onClick={toggleTheme} 
          className="w-10 h-10 rounded-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-md flex items-center justify-center text-slate-600 dark:text-yellow-400 hover:scale-110 transition-all"
          title={theme === 'dark' ? "Mudar para Modo Claro" : "Mudar para Modo Escuro"}
        >
          {theme === 'dark' ? <i className="fa-solid fa-sun text-lg"></i> : <i className="fa-solid fa-moon text-lg"></i>}
        </button>
      </div>
